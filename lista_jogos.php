...
if ($genero) {
    $count_sql = "SELECT COUNT(*) AS total FROM games 
                  WHERE (nome_jogo LIKE '%$pesquisa%' OR desc_jogo LIKE '%$pesquisa%' OR generos LIKE '%$pesquisa%') 
                  AND generos LIKE '%$genero%'";
} else {
    $count_sql = "SELECT COUNT(*) AS total FROM games 
                  WHERE nome_jogo LIKE '%$pesquisa%' OR desc_jogo LIKE '%$pesquisa%' OR generos LIKE '%$pesquisa%'";
}
$resultado = $conecta->query($count_sql);

$quant_resultado = 0;
if ($resultado) {
    $linha = $resultado->fetch_assoc();
    $quant_resultado = (int) $linha['total'];
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <?php if ($pesquisa != "" || $genero != ""): ?>
                    <?php if ($quant_resultado > 0): ?>
                        <p style="color:white; font-size:25px; text-align:center; font-weight:bold;font-style:italic;">
                            Sua pesquisa retornou <span style="text-decoration:underline"><?php echo $quant_resultado ?></span> resultado(s)
                        </p>
                    <?php else: ?>
                    <?php endif; ?>
                <?php else: ?>
                    <p style="color:white; font-size:25px; text-align:center; font-weight:bold;font-style:italic;">
                        Temos atualmente <span style="text-decoration:underline"><?php echo $quant_resultado ?></span> jogos registrados
                    </p>
                <?php endif; ?>
...
if (isset($_GET["genero"])) {
    ...
    $sql = "SELECT img_jogo, generos, desc_jogo, nome_jogo, id_jogo, horario_postado, avaliacao_media FROM games WHERE (nome_jogo LIKE '%$pesquisa%' OR desc_jogo LIKE '%$pesquisa%' OR generos LIKE '%$pesquisa%') AND generos LIKE '%$genero%' order by id_jogo ASC";

    $stmt = $conecta->prepare($sql);
    $stmt->execute();
    $resultado = $stmt->get_result();
} else {
     $sql = "SELECT img_jogo, generos, desc_jogo, nome_jogo, id_jogo, horario_postado, avaliacao_media FROM games WHERE nome_jogo LIKE '%$pesquisa%' OR desc_jogo LIKE '%$pesquisa%' OR generos LIKE '%$pesquisa%' order by id_jogo ASC";
  ...
  <button type='submit' class='btn btn-primary mx-auto vermais' style='font-weight:bold; font-style:italic;  text-shadow: 2px 2px #000; font-size:18px; margin:-13px; background-color:darkslategrey; border:1px solid darkslategrey;'>Ver detalhes</button>
        ...
        if (isset($nada)) {
            echo "<br><br><div style='text-align:center; margin-top:3%; margin-left:4%;'  id='demoFont'>Desculpe :( <br><p></p> Sua pesquisa retornou 0 resultados.</div> ";
            unset($nada);
        }
        ?>
        <br>
